...
public class OrderService {
    ...
    public OrderResponse createOnlineOrder(CreateOrderRequest request, String authorization) {
        ...
        if ("SELLER".equals(authUser.getRole())
                ...
                && authUser.getStoreId().equals(request.getStoreId())) {
            throw new BadRequestException("Seller cannot buy from their own store");
        }
    ...
    public List<OrderResponse> findMyOrders(String authorization) {
        ...
        if ("CUSTOMER".equals(authUser.getRole())) {
            return findMyPurchasesInternal(authUser);
        }
        ...
        if ("SELLER".equals(authUser.getRole())) {
            ...
            return salesOrderRepository
                    .findByStoreIdOrCustomerIdOrderByCreatedAtDesc(authUser.getStoreId(), authUser.getId())
                    .stream()
    ...
    @Transactional(readOnly = true)
    public List<OrderResponse> findMySales(String authorization) {
        AuthUserResponse authUser = authIdentityService.requireSeller(authorization);
    ...
    @Transactional(readOnly = true)
    public List<OrderResponse> findMyPurchases(String authorization) {
        AuthUserResponse authUser = authIdentityService.requireCustomerOrSeller(authorization);
    ...
    @Transactional(readOnly = true)
    public PageResponse<OrderResponse> findMyOrdersPage(String authorization, int page, int size) {
        AuthUserResponse authUser = authIdentityService.requireCustomerOrSeller(authorization);
        Pageable pageable = PageRequest.of(page, size);

        if ("CUSTOMER".equals(authUser.getRole())) {
            return PageResponse.from(
                    salesOrderRepository.findByCustomerIdOrderByCreatedAtDesc(authUser.getId(), pageable).map(OrderMapper::toResponse)
            );
        }

        return PageResponse.from(
                salesOrderRepository.findByStoreIdOrCustomerIdOrderByCreatedAtDesc(authUser.getStoreId(), authUser.getId(), pageable)
                        .map(OrderMapper::toResponse)
        );
    }

    @Transactional(readOnly = true)
    public PageResponse<OrderResponse> findMySalesPage(String authorization, int page, int size) {
        AuthUserResponse authUser = authIdentityService.requireSeller(authorization);
        Pageable pageable = PageRequest.of(page, size);
        return PageResponse.from(
                salesOrderRepository.findByStoreIdOrderByCreatedAtDesc(authUser.getStoreId(), pageable).map(OrderMapper::toResponse)
        );
    }

    @Transactional(readOnly = true)
    public PageResponse<OrderResponse> findMyPurchasesPage(String authorization, int page, int size) {
        AuthUserResponse authUser = authIdentityService.requireCustomerOrSeller(authorization);
        Pageable pageable = PageRequest.of(page, size);
        return PageResponse.from(
                salesOrderRepository.findByCustomerIdOrderByCreatedAtDesc(authUser.getId(), pageable).map(OrderMapper::toResponse)
    ...
    private List<OrderResponse> findMyPurchasesInternal(AuthUserResponse authUser) {
    ...
    private void assertOrderAccess(AuthUserResponse authUser, SalesOrder order) {
        if ("SELLER".equals(authUser.getRole())) {
            boolean isStoreSale = authUser.getStoreId() != null && authUser.getStoreId().equals(order.getStoreId());
            boolean isPersonalPurchase = order.getCustomerId() != null && authUser.getId().equals(order.getCustomerId());

            if (!isStoreSale && !isPersonalPurchase) {
